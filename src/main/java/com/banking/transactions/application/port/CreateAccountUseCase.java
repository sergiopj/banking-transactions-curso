 ...
 * aplicación, no CÓMO. Es el contrato que el exterior (Controller) ve.
 * Principio ISP de
 * SOLID.
 * 
 * POR QUÉ public interface y no class: Es el patrón Puerto y Adaptador. El
 * Controller (adapter de entrada) solo conoce esta interface, no la
 * implementación
 * concreta que vive en application/service/CreateAccountService. Así puedes
 * cambiar
 * la lógica sin tocar el web.
 * 
 * POR QUÉ UseCase y no Service: Semántica DDD/Hexagonal. Service es genérico y
 * de Spring. UseCase dice "esta interface es UN caso de uso del negocio", no un
 * servicio técnico. Es lenguaje ubicuo.
 * 
 * POR QUÉ createAccount(CreateAccountCommand): Recibe un COMMAND de entrada
 * (Write Model) y no Account ni primitivos sueltos. Así el método tiene un solo
 * parámetro con
